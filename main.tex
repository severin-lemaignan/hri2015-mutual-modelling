...
\begin{figure}[h!t]
        ...
        \caption{The False-Beliefs experiment: two puppets, ``Ann'' and
...
that $B$ knows $X$''.  This is a straightforward but useful reduction to reason
on mutual modelling.  This notation does not mean $A$ has an explicit,
monolithic representation of $B$: it must be understood as an abstraction
referring to complex socio-cognitive processes. Besides, we refer to the
...
\begin{enumerate}
    ...
    \item Tasks vary a lot with respect to how much they require mutual
        understanding.  The \emph{grounding criterion}
        \groundingcriterion~\cite{clark1986referring} refers to
    ...
    \item Before any specific grounding action, there is usually a non-null
        probability that $X$ is mutually understood by $A$ and $B$: because $X$
        is part of $A$'s and $B$'s cultures, because it is manifest to co-present
        subjects or simply because there is not much space for misunderstanding
        or disagreement about $X$. We could not simply collaborate without a
        high level of initial grounds. We note the theoretical accuracy of
...
during interactions (hereafter, $H$ stands for the human, while $R$ stands for
the robot). Figure~\ref{mm_symmetry} represents a dyadic human-robot
interaction, with $\Delta_1$ illustrating the \emph{symmetry} question ({\it is
the accuracy of my model related or not to the accuracy of your model?}).
...
Finally, this model also suggest a \emph{rectangle question}: how self-
versus other modelling compares ($\Delta_4$ in
Figure~\ref{mm_rectangle})? This gives an indication of meta-cognitive skills of
the agents. We can also question if the modelling skills depend upon what
aspects are being modeled ($X$ or $Y$) which would explain vertical differences ($\Delta_5$ in
...
This model, \emph{designed in the context of human collaboration}, represents
our scientific starting point to formalize mutual modelling in human-robot
interaction.
