...
applied to \emph{social reasoning}, and he underlines both the limits of
epistemic logic for that purpose (it assumes for instance $\mathsf{K}_{i\varphi}
\rightarrow \mathsf{K}_{i}\mathsf{K}_{i\varphi}$, which reads ``$i$ knows
$\varphi$'' implies ``$i$ knows that $i$ knows $\varphi$'', \ie $i$ can always
introspect) and the recent advancement towards modeling \emph{human} social
cognition, which implies for instance limited rationality.


\emph{Doxastic epistemic logic} proposed by van Ditmarsch and
Labuschagne in~\cite{vanditmarsch2007beliefs} that has been shown to be suitable
